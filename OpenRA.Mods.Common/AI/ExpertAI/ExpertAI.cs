...
namespace OpenRA.Mods.Common.AI
{
	public sealed class ExpertAIInfo : IBotInfo, ITraitInfo
	{
		public class UnitCategories
		{
			...
			public readonly HashSet<string> Healer = new HashSet<string>();
	...
	public sealed class ExpertAI : ITick, IBot, INotifyDamage
	{
		...
		public int MaxBaseRadius { get { return Info.MaxBaseRadius; } }
		...
		public CPos? ChooseBuildLocation(string actorType, bool distanceToBaseIsImportant, BuildingPlacementType type)
		{
			...
			switch (type)
			{
				...
				case BuildingPlacementType.Refinery:
					...
					var nearbyMines = World.FindActorsInCircle(Map.CenterOfCell(baseCenter), WDist.FromCells(Info.MaxBaseRadius))
						.Where(a => a.Owner.NonCombatant && a.Info.HasTraitInfo<SeedsResourceInfo>()).ToList();
					...
					nearbyMines.RemoveAll(m =>
						World.FindActorsInCircle(m.CenterPosition, WDist.FromCells(Info.MaxBaseRadius))
							.Where(IsEnemyUnit).Count() > 5);

					if (nearbyMines.Any())
					{
						var found = FindPos(baseCenter, nearbyMines.ClosestTo(baseCenter).Location,
							Info.MinBaseRadius, Info.MaxBaseRadius, actorType, bi, distanceToBaseIsImportant);
						if (found != null)
							return found;
					}
		...
		{
			var mines = World.ActorsHavingTrait<SeedsResource>().Where(m => m.Owner.NonCombatant);
			if (!mines.Any())
				return null;
			...
			CVec front = mcv.Location - bestMine.Location;
		...
		void SellUselessRefinery()
		{
			...
			foreach (var r in refs)
			{
				...
				if (havestableCellsNear < 0) // We got -1 == No harvester. Don't sell!
					return;
			...
			// Examine toSell and sell the worst one.
			var mines = World.ActorsHavingTrait<SeedsResource>().Where(a => a.Owner.NonCombatant);
			if (!mines.Any())
				return; // doesn't really happen on sane maps

			var badRef = toSell.MaxBy(r => (mines.ClosestTo(r).CenterPosition - r.CenterPosition).LengthSquared);
			QueueOrder(new Order("Sell", badRef, false) { TargetActor = badRef });
		}
		...
		{
			return Info.BuildingCommonNames.Defense.Contains(a.Info.Name);
		}

		public bool IsSeigeUnit(Actor a)
		{
			return Info.UnitsCommonNames.Seige.Contains(a.Info.Name);
		}

		public bool IsFriendlyUnitTargeter(Actor a)
		{
			return Info.UnitsCommonNames.Healer.Contains(a.Info.Name);
		}

		public bool IsNavalBuilding(string name)
		{
			return Info.BuildingCommonNames.NavalProduction.Contains(name);
		}
